...
export const API_BASE = (import.meta.env.VITE_API_URL as string | undefined) || "https://complaint-backend-vk5t.onrender.com/api/v1";
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !original?._retry && !String(original?.url ?? "").includes("/auth/")) {
      ...
      isRefreshing = true;
      try {
        const refreshed = await axios.post(`${API_BASE}/auth/refresh`, {}, { withCredentials: true });
